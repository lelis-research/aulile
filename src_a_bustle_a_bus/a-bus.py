    ...
    def is_partial_solution(self, p):
        ...
        if is_partial_program:
            ...
            for inout in self._input_output:
                ...
                if len(out) > len(inout['out']):
                    eval_scores.append(distance(out[: int(len(out) * lev_factor)], inout['out']))
                else:
                    eval_scores.append(distance(out, inout['out'][: int(len(inout['out']) * lev_factor)]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--log",
        dest="log_filename",
        default="BUS_Augmented_sygus.log",
        type=str,
        help="Log filename - if not specified, defaults to BUS_Augmented_sygus.log",
    )
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="Levenshtein Factor. If not specified, defaults to 1.",
    )
    ...
    global lev_factor
    lev_factor = parameters.lev_factor
