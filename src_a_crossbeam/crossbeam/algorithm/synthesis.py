...
from crossbeam.dsl.value import LibraryValue
from crossbeam.dsl.domains import Domain
...
def synthesize_wrapper(*args, **kwargs):
    # attempts are not deterministic
    attempts = kwargs["attempts"]
    grow_library = kwargs["grow_library"]
    stop_if_no_grow = kwargs["stop_if_no_grow"]
    task_timeout = kwargs["task_timeout"]
    attempt_timeout = kwargs["attempt_timeout"]
    assert task_timeout >= attempt_timeout
    del kwargs["task_timeout"]
    del kwargs["attempt_timeout"]
    del kwargs["attempts"]
    del kwargs["grow_library"]
    del kwargs["stop_if_no_grow"]
    ...
    time_left = task_timeout
    used_vals = []
    init_values(task, domain, used_vals)
    used_vals = set(used_vals)
    global_best_score = float("inf")
    for _ in  range(attempts):
        if time_left and time_left <= 0:
            break
        kwargs["timeout"] = min(time_left, attempt_timeout)
        start_time = timeit.default_timer()
        out, all_values, stats, library = synthesize(*args, **kwargs)
        synth_time = timeit.default_timer() - start_time
        time_left -= synth_time
        all_out.append(out)
        all_all_values.append(all_values)
        all_stats.append(stats)
        all_times.append(synth_time)
        ...
        if grow_library:
            ...
            best_value = None
            n_outputs = len(output_value.values)
            for value in all_values:
                # check if at least one output matches the ground truth
                if value in used_vals:
                    continue
                is_partial = False
                for val_out, ground_out in zip(value.values, output_value.values):
                    if val_out == ground_out:
                        is_partial = True
                        break
                if not is_partial:
                    continue

                # compute score for the 'value' (partial program)
                score = sum([distance(str(val_out), str(ground_out)) for val_out, ground_out in
                             zip(value.values, output_value.values)]) / n_outputs
                if score < best_score and isinstance(value, value_module.OperationValue):
                    best_score = score
                    best_value = value

            if best_value:
                kwargs["library"].append(LibraryValue(best_value.values, best_value.operation, best_value.arg_values, 1))
                used_vals.add(best_value)
            else:
                if stop_if_no_grow:
                    break
                kwargs["library"].append(None)
        
        if global_best_score > best_score:
            global_best_score = best_score
        else:
            break
    return all_out, all_all_values, all_stats, all_times, kwargs["library"]
...
def synthesize(task, domain, model, device,
               ...
               random_beam=False, use_ur=False, masking=True, static_weight=False, library=[]):
  ...
  while True:
    ...
    for operation in domain.operations:
      if (end_time is not None and timeit.default_timer() > end_time) or (
        ...
        return None, all_values, stats, library
      if verbose:
      ...
      if use_ur:
        ...
        for new_value in new_values:
          ...
          if new_value == output_value:
            return new_value, all_values, stats, library
      ...
      for i, arg_list in enumerate(beam):
        ...
        if result_value == output_value and not is_training:
          return result_value, all_values, stats, library
      ...
      if is_training and len(trace) and trace[0].operation == operation:
        ...
        trace.pop(0)
        if len(trace) == 0:
          return training_samples, all_values, stats, library
