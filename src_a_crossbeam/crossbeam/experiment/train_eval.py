...
def do_eval(eval_tasks, domain, model,
            ...
            task_timeout=None, attempt_timeout=None, max_values_explored=None, is_stochastic=False, use_ur=True,
            use_type_masking=True, static_weight=False, attempts=1, grow_library=False, stop_if_no_grow=False):
  if verbose:
    print('doing eval...')
  ...
  for t in eval_tasks:
    ...
    with torch.no_grad():
      all_out, all_all_values, all_stats, all_times, library = synthesis.synthesize_wrapper(
          ...
          stop_if_no_grow=stop_if_no_grow,
          attempts=attempts)
    ...
    for i in range(len(all_out)):
        ...
        json_dict['results'].append({
            ...
            'ending_library': [l.expression() if l else None for l in library[:i + 1]] if library
            else None,
        })
        if verbose:
          print(f"Attempt: {i + 1}")
          ...
          if not out and library and not library[i]: # library can only grow after 1st failed attempt
              print(f"Failed to grow library on attempt {i + 1}")
          print(f"Ending Library: {[l.expression() if l else None for l in library[:i + 1]] if library else None}")
          sys.stdout.flush()
        if out:
          num_tasks_solved += 1
          assert i + 1 == len(all_out)
        elif i + 1 == len(all_out):
  ...
  json_dict["total_time"] = total_time

  return success_rate, json_dict, failed_tasks
...
def train_eval_loop(args, device, model, train_files, eval_tasks,
  ...
  eval_func = functools.partial(do_eval,
                                ...
                                grow_library=args.grow_library,
                                stop_if_no_grow=args.stop_if_no_grow,)
  if args.do_test: # test only
    assert args.num_proc == 1
    print('Doing test only!')
    succ, json_dict, failed_tasks = eval_func(eval_tasks, domain, model, verbose=not is_distributed)
    if args.json_results_file:
      with open(args.json_results_file, 'w') as f:
        json.dump(json_dict, f, indent=4, sort_keys=True)
      print('Wrote JSON results file at {}'.format(args.json_results_file))
    if failed_tasks:
      failed_tasks_file = args.json_results_file.split('.json')[0] + '_failed_tasks.txt'
